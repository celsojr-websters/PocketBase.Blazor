...
public class CronGeneratorTests
{
    ...
    {
        // Arrange
        string tempDir = Path.Combine(Path.GetTempPath(), "pb_crons_test");
        ...
        // Assert
        string mainFile = Path.Combine(tempDir, "main.go");
        string registryFile = Path.Combine(tempDir, options.OutputDirectory, "registry.go");
        string handlersFile = Path.Combine(tempDir, options.OutputDirectory, "handlers.go");
        string typesFile = Path.Combine(tempDir, options.OutputDirectory, "types.go");
        string runtimFile = Path.Combine(tempDir, options.OutputDirectory, "runtime.go");
        ...
        string handlersContent = await File.ReadAllTextAsync(handlersFile);
    ...
    {
        // Arrange
        string tempDir = Path.Combine(Path.GetTempPath(), "pb_crons_test_default");
        if (Directory.Exists(tempDir)) Directory.Delete(tempDir, true);
